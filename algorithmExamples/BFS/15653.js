let input = require('fs').readFilrSync('/dev/stdin').toString().split('\n')
...
//0. visited 배열
for(let i = 0; i < area[0]/1; i++){
    visited.push(Array(area[2]/1).fill(false));
}
for(let i = 0; i < area[0]/1; i++){
    for(let j = 0; j < area[2]/1; j++){
        visited[i][j] = visited.map(v => v.slice())
    }
}
//1. R, B 찾기
for(let i = 1, x = area[0]/1; i < x-1; i++){
...
function BFS(){
    ...
    while(queue.length > 0){
        ...
        for(let i = 0; i < 4; i++){
            ...
            if( input([nbxy[0],nbxy[1]]) === 'O') continue;
            ...
            if(visited[nrxy[0]][nrxy[1]][nbxy[0]][nbxy[1]]){
                visited[nrxy[0]][nrxy[1]][nbxy[0]][nbxy[1]] = true;
